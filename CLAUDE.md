...
- **`title` obligatorio en todo enlace e imagen, y `alt` en toda imagen.** `AppLink` lo exige por tipo; `Media` usa el `alt` si no se pasa. El test `e2e/titles.spec.ts` bloquea cualquier `<a>` o `<img>` sin título y cualquier `<img>` sin alt: en este sitio no hay imágenes decorativas, las que lo serían se pintan con CSS, así que un alt vacío siempre es un descuido. `cursor: pointer` es global para `a`, `button`, `summary` y `[role=button]`.
...
- **Ficha de socios fundadores en `/nosotros`.** La foto de los dos (`/fundadores/fundadores.webp`, 626x540) encabeza su propia tarjeta, a proporción real y sin recorte, así que `sizes` es el ancho de la caja. Los retratos pequeños salen de `teamData` y viven en `/equipo/` porque el schema lo exige (`photo` empieza por `/equipo/`); la caja es cuadrada y la foto de Iván apaisada, por eso `sizes` declara 96px y no 64.
- **Casilla de verificación propia** (`Checkbox` en `Field`): la nativa no se puede redondear ni suavizar el check. El `<input type="checkbox">` sigue siendo real, solo lleva `appearance-none`, y el check es un SVG hermano que aparece con `peer-checked` creciendo desde el centro. Teclado, formulario y lectores de pantalla no cambian.
- **El botón del campo de archivo va a ras del borde y a toda la altura** (`file:h-12`, `py-0 pl-0` en el input). Con el padding vertical del control, al botón nativo no le cabía su propia altura y el texto se recortaba. Su etiqueta la pone el navegador según su idioma, no se puede cambiar.
- **Crédito de la agencia en el pie** (Podio Chile, enlace externo a podiochile.com). El `.svg` que entregó el cliente es solo un envoltorio con dos PNG de 1600x260 dentro y pesa 239 KB; se rasteriza una vez a `/autor/logo-podio-chile.webp` (28 KB, con alfa) y es lo que sirve la web. La opacidad de reposo va **solo sobre el logo**: aplicada al texto bajaba el contraste a 4,06 y axe lo rechaza.
